...
(function () {
  ...
  const dom = {
    ...
    cameraSelectWrap: $("cameraSelectWrap"),
  ...
  /* ------------------------------------------------------------------
     13. SUPPORTED CODE FORMATS + CAMERA MEMORY
     Based on the official html5-qrcode capabilities: the underlying
     Zxing-js decoder can read QR codes AND common retail barcodes
     (EAN/UPC/CODE/ITF/etc). Restricting to a sensible, well-supported
     subset — rather than scanning against every exotic format — keeps
     decoding fast on phones. MAXICODE / RSS_14 / RSS_EXPANDED are left
     out since they're rare and not covered by the fast native
     BarcodeDetector path either.
     All of this runs purely in the browser — nothing here needs
     anything beyond an Android phone + Chrome to build or deploy.
  ------------------------------------------------------------------ */
  function getSupportedFormats() {
    ...
    return [
      ...
      F.DATA_MATRIX, F.ITF, F.EAN_13, F.EAN_8, F.PDF_417,
      F.UPC_A, F.UPC_E, F.UPC_EAN_EXTENSION,
    ].filter((v) => v !== undefined);
  }

  function buildHtml5QrOptions() {
  ...
  const CAMERA_PREF_KEY = "qrv-preferred-camera-id";
  function loadPreferredCameraId() {
    try { return localStorage.getItem(CAMERA_PREF_KEY); } catch (e) { return null; }
  }
  function savePreferredCameraId(id) {
    try { localStorage.setItem(CAMERA_PREF_KEY, id); } catch (e) { /* storage unavailable — non-critical */ }
  }
  ...
  async function listCameras() {
    ...
    // Restore the camera the user picked last time (stored locally on-device only).
    const preferredId = loadPreferredCameraId();
    if (preferredId) {
      const idx = state.cameras.findIndex((c) => c.id === preferredId);
      if (idx !== -1) state.activeCameraIndex = idx;
    }
  ...
  function populateCameraSelect() {
    if (state.cameras.length < 2) {
      dom.cameraSelectWrap.hidden = true;
      return;
    }
    dom.cameraSelectWrap.hidden = false;
    dom.cameraSelect.innerHTML = "";
    state.cameras.forEach((cam, idx) => {
      const opt = document.createElement("option");
      opt.value = String(idx);
      // Most devices label cameras like "back camera" / "front camera" — fall back to a generic name.
      const label = cam.label || `Camera ${idx + 1}`;
      opt.textContent = /back|rear/i.test(label) ? `Rear Camera — ${label}` : /front|user/i.test(label) ? `Front Camera — ${label}` : label;
      dom.cameraSelect.appendChild(opt);
    });
    dom.cameraSelect.value = String(state.activeCameraIndex);
  }

  dom.cameraSelect.addEventListener("change", async () => {
    const idx = parseInt(dom.cameraSelect.value, 10);
    if (Number.isNaN(idx) || idx === state.activeCameraIndex) return;
    state.activeCameraIndex = idx;
    if (state.cameras[idx]) savePreferredCameraId(state.cameras[idx].id);
  ...
  function updateFlipLabel() {
    ...
    const label = state.cameras[state.activeCameraIndex].label || "";
    const isFront = /front|user/i.test(label);
    dom.flipCameraLabel.textContent = isFront ? "Switch to Rear" : "Switch to Front";
  }
  ...
  async function startCamera() {
    ...
    try {
      if (!state.html5QrCode) {
        state.html5QrCode = new Html5Qrcode("cameraReader", buildHtml5QrOptions());
      }
      ...
      const scanConfig = {
        fps: 15,
        qrbox: (vw, vh) => {
          // Larger capture box = easier to fill the frame with a QR at varying distances.
          const size = Math.floor(Math.min(vw, vh) * 0.78);
          return { width: size, height: size };
        },
        aspectRatio: 1.0,
        disableFlip: false,
        // Uses the browser's native, hardware-accelerated barcode detector when available —
        // noticeably faster and more reliable than the pure-JS fallback decoder.
  ...
  async function flipCamera() {
    ...
    dom.cameraSelect.value = String(state.activeCameraIndex);
    savePreferredCameraId(state.cameras[state.activeCameraIndex].id);
    await stopCamera();
    await startCamera();
  }
  ...
  ------------------------------------------------------------------ */
  function setUploadStatus(text) { dom.uploadStatus.textContent = text || ""; }
  ...
  function handleFiles(fileList) {
    ...
    decodeImageWithFallbacks(file)
      .then((decodedText) => {
        setUploadStatus("QR code detected ✓");
        handleDecodedText(decodedText);
        setTimeout(() => setUploadStatus(""), 2000);
      })
      .catch(() => {
        setUploadStatus("");
        toast("Is image mein QR code detect nahin ho paaya.");
        dom.uploadTips.hidden = false;
      });
  }

  /**
   * Attempts to decode a QR code from an uploaded image using several
   * progressively-modified variants of the same picture. Real-world photos
   * (angled labels, product stickers, low-contrast prints) often fail on a
   * single naive attempt, so each variant targets a different common failure
   * mode: scale, rotation, and contrast/lighting.
   *
   * Resolves with the decoded text from the FIRST variant that succeeds.
   * Rejects only if every variant fails.
   */
  function decodeImageWithFallbacks(file) {
    return new Promise((resolve, reject) => {
      ...
      img.onload = async () => {
        ...
        let variants;
        try {
          variants = buildImageVariants(img);
        } catch (e) {
          reject(e);
          return;
        }
        ...
        for (let i = 0; i < variants.length; i++) {
          setUploadStatus(`Scanning… attempt ${i + 1} of ${variants.length}`);
          try {
            const blob = await variants[i]();
            const text = await scanBlobForQr(blob);
            resolve(text);
            return;
          } catch (e) {
            // this variant found nothing — move on to the next
          }
        }
        reject(new Error("No QR code found in any variant"));
      };
  ...
  // Runs html5-qrcode's file scanner against a single image Blob.
  function scanBlobForQr(blob) {
    ...
    const scanner = new Html5Qrcode(tempId, buildHtml5QrOptions());
    const asFile = new File([blob], "variant.png", { type: "image/png" });
    return scanner.scanFile(asFile, false).finally(() => {
      try { scanner.clear(); } catch (e) { /* noop */ }
    });
  }

  // Builds an ordered list of lazy canvas->Blob producers, each a different
  // preprocessing pass over the same source image.
  function buildImageVariants(img) {
    ...
    // 1. Original, as-is — the fast path for already-clean QR images.
    variants.push(() => {
    ...
    // 2. Upscaled 2x — helps small or distant QR codes where modules are only a few pixels wide.
    variants.push(() => {
      const scale = 2;
      const c = document.createElement("canvas");
    ...
    // 3. Grayscale + contrast boost — helps QR codes printed on shiny, low-contrast,
    //    or unevenly lit labels (common on real product packaging photographed at an angle).
    variants.push(() => {
      const c = document.createElement("canvas");
      c.width = w; c.height = h;
      const ctx = c.getContext("2d");
    ...
    // 4. Center crop, zoomed — for photos where the QR only occupies a small central portion.
    variants.push(() => {
      const cropSize = Math.floor(Math.min(w, h) * 0.6);
      const sx = Math.floor((w - cropSize) / 2);
      const sy = Math.floor((h - cropSize) / 2);
      const c = document.createElement("canvas");
      c.width = cropSize * 2; c.height = cropSize * 2;
      const ctx = c.getContext("2d");
      ctx.imageSmoothingEnabled = true;
      ctx.drawImage(img, sx, sy, cropSize, cropSize, 0, 0, c.width, c.height);
      return toBlob(c);
    });

    // 5–7. Rotations — covers QR codes photographed sideways or upside-down.
  ...
  /* ------------------------------------------------------------------
     SAMPLE / DEMO RESULT
     Lets a first-time user instantly see what a full security report
     looks like, without needing a physical QR code on hand.
  ------------------------------------------------------------------ */
  const SAMPLE_QR_TEXT = "https://paym3nt-secure-verify.tk/login?user=reward&otp=confirm";

  dom.btnTrySample.addEventListener("click", () => {
    handleDecodedText(SAMPLE_QR_TEXT);
    toast("Yeh ek sample/demo result hai — asli QR scan nahin hai.");
  });
  ...
  function parseQRContent(raw) {
    ...
    // --- Retail Barcode (EAN-8 / EAN-13 / UPC-A / UPC-E / ITF-14) ---
    // Product barcodes decode to a plain run of digits with no scheme prefix,
    // so they're distinguished purely by length matching a known standard.
    if (/^\d{6,14}$/.test(text)) {
      result.type = "barcode";
      result.typeLabel = "Product Barcode";
      const barcodeFormats = { 8: "EAN-8", 12: "UPC-A", 13: "EAN-13", 14: "ITF-14 / GTIN-14" };
      const format = barcodeFormats[text.length] || `${text.length}-digit numeric code`;
      result.purpose = "Retail product identification number — used by store checkout (POS) systems to look up a product.";
      result.fields = {
  ...
  function computeRisk(parsed) {
    ...
    if (parsed.type === "barcode") {
      addFlag("info", 0, "This is a standard retail product barcode — it contains no link, payment request, or personal data. Generally completely safe.");
    }
  ...
  function buildExplanation(parsed, risk, brand) {
    ...
    switch (parsed.type) {
      ...
      case "location":
        ...
        break;
      case "barcode":
        sentences.push(`This is a retail product barcode (${parsed.fields["Barcode Format"]}) — the kind printed on packaged goods. It just identifies a product for a store's checkout system, not a link or payment.`);
        break;
      case "crypto":
  ...
  dom.btnShare.addEventListener("click", async () => {
    ...
    const shareText = `QR Verify report\nType: ${parsed.typeLabel}\nRisk: ${risk.score}/100 (${risk.level})\nContent: ${state.currentAnalysis.raw}`;
    if (navigator.share) {
      try { await navigator.share({ title: "QR Verify Report", text: shareText }); }
  ...
  function generatePdfReport(analysis) {
    ...
    doc.text("QR Verify — Security Report", margin, y);
    y += 18;
    ...
    doc.text("Generated entirely on-device by QR Verify — no data was uploaded or stored. Created by Imtiyaz Surjapuri.", margin, 812);
